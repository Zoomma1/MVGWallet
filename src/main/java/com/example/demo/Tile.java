...
public class Tile {
    ...
    public Node displayTile(){
        ...
        vBox.setOnMouseClicked(event -> {
            try {
                TokenPageController.setCoin(symbol,price);
    ...
    public Node displayStockTile(){
        ...
        yAxis.setAutoRanging(false);
        yAxis.setUpperBound(maxValue * 1.01);
        yAxis.setLowerBound(minValue * 0.99);
        ...
        vBox.setOnMouseClicked(event -> {
            try {
                SharePageController.setCoin(symbol,price);

                FXMLLoader loader = new FXMLLoader(getClass().getResource("SharePage.fxml"));
                root = loader.load();
                stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
